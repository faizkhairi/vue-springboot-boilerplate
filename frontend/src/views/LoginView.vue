...
async function submit() {
  ...
  try {
    ...
    error.value = err.response?.status === 401 ? 'Invalid email or password.' : 'Login failed.'
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="min-h-screen flex items-center justify-center bg-gray-50 p-4">
    <div class="w-full max-w-md rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
      ...
      <p class="text-gray-600 mt-1">Enter your email and password.</p>
      <form @submit.prevent="submit" class="mt-4 space-y-4">
        ...
        <p class="text-center text-sm text-gray-600">
          Don't have an account? <router-link to="/register" class="font-medium text-gray-900 underline">Register</router-link>
        </p>
      </form>
    </div>
  </div>
